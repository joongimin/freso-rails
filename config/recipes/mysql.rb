namespace :mysql do
  ...
  task :backup, :roles => :db do
    run "mysqldump --opt -u#{mysql_user} -p#{mysql_password} #{mysql_database} -h #{mysql_host} > #{shared_path}/backups/#{release_name}.sql"
  end
  ...
  task :sync, :roles => :db do
    if rails_env == "test" || rails_env == "development"
      ...
      run "mysqldump -u#{mysql_user} -p#{mysql_password} #{mysql_database} -h #{mysql_host} > #{shared_path}/backups/#{mysql_dumpfile}"
